...
const Step1 = ({ data, setData, schools }) => {
  const rayonlar = [...new Set(schools.map(kg => kg.rayon))];
  const mekteblerInRayon = schools.filter(kg => kg.rayon === data.rayon);
  const handleChange = (e) => {
    const { name, value } = e.target;
    if (name === 'rayon') { setData({ ...data, rayon: value, mektebId: '' }); } 
  ...
  return (
    <div>
      ...
      <div className="form-group"><label>Müəssisəni seçin *</label><select name="mektebId" value={data.mektebId} onChange={handleChange} required disabled={!data.rayon}><option value="">Müəssisə seçin...</option>{mekteblerInRayon.map(b => <option key={b.id} value={b.id}>{b.adi}</option>)}</select></div>
    </div>
  );
};
        ...
        <div>
            ...
            {data.signatures.map((sig, index) => (
                <div className="signature-block" key={index}>
                    ...
                    <label>İmza:</label>
                    <SchoolSignaturePad onSignatureEnd={(imzaData) => onSignatureChange(index, 'imzaData', imzaData)} />
                </div>
            ))}
        </div>
    );
...
const SchoolMonitoringForm = () => {
  ...
  const getInitialState = () => ({
    rayon: '', mektebId: '', usaqTutumu: '', mtisUsaqSayi: '', sifarisEdilenQida: '', faktikiUsaqSayi: '',
  ...
  useEffect(() => {
    ...
    localStorage.setItem('newSchoolMonitoringFormDraft', JSON.stringify(dataToSave));
  }, [formData]);
  ...
  useEffect(() => {
    const fetchschools = async () => {
      setIsLoading(true);
      const querySnapshot = await getDocs(collection(db, "mektebler"));
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1: return <Step1 data={formData} setData={setFormData} schools={schools} />;
      ...
      default: return <Step1 data={formData} setData={setFormData} schools={schools} />;
    }
  };
  ...
  return (
    <div className="new-monitoring-form-container">
      <div className="form-top-bar">
          <div className="form-main-title">Təhsil müəssisəsində monitorinq aparılması</div>
